...
class ExecutionWatchdog:
    ...
    def build_procedures_bootstrap_code(self):
        return (
            ...
            "\n"
            "# Hyde Metadata Notification API (First Pass)\n"
            "from ipykernel.comm import Comm\n"
            "import numpy as np\n"
            "__hyde_comm = Comm(target_name='hyde_mgmt', data={})\n"
            "\n"
            "def __hyde_notify_namespace():\n"
            "    # Filter out internal/module names for a cleaner view\n"
            "    view = {}\n"
            "    for name, obj in __main__.__dict__.items():\n"
            "        if name.startswith('_') or name in ['os', 'sys', 'importlib', 'procedures']:\n"
            "            continue\n"
            "        \n"
            ...
            "            value = ''\n"
            "            \n"
            "        view[name] = {'type': obj_type, 'value': value}\n"
            "    \n"
            "    __hyde_comm.send({'type': 'NAMESPACE_UPDATE', 'data': view})\n"
            "\n"
            "get_ipython().events.register('post_execute', __hyde_notify_namespace)\n"
            "__hyde_notify_namespace()  # Initial notification\n"
        )
