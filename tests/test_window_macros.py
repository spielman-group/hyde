...
class TestWindowMacroStore(unittest.TestCase):
    ...
    def test_overwrite_existing_function_in_place(self):
        ...
        updated = update_macro_source(path, "Table0", macro)
    ...
    def test_write_macro_source_rejects_user_function_before_autogenerated_duplicate(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            procedures_init = os.path.join(tmpdir, "__init__.py")
            original = (
                "import hyde\n\n"
            ...
            conflict = inspect_macro_conflict(procedures_init, "Table0")
            self.assertIsNotNone(conflict)
            self.assertFalse(conflict["in_autogenerated_block"])
            self.assertTrue(conflict["multiple_matches"])
