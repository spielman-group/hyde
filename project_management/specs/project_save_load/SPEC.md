...
- It then dispatches hidden `hyde.save_project(target, mode="save_as", overwrite=True)`.
...
- If the target exists and is non-empty, Hyde asks for confirmation before overwriting
  the existing project contents.
- It then dispatches hidden `hyde.save_project(target, mode="copy", overwrite=True)`.
