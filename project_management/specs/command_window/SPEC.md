...
- [ ] Show runtime-helper-session output in the command window without echoing runtime-helper input. Note: this is likely a backend requirement presented to the user via the command window.
...
- [ ] Document and implement the final policy for other non-user runtime-helper traffic, such as figure-preview updates.
...
- Initial loading and reload of `procedures/__init__.py` are owned by the GUI-owned runtime helper, not the command window.
