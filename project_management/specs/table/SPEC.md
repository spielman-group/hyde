...
The table follows Hyde's IR-control pattern:

- `TableWidget.widget_ir` owns the live `TableIR` used for table construction,
  recreation, layout capture, background table requests, and live table-adjacent
  mutation commands
- `TableIRDiff` is derived from those IRs for change-oriented table behavior
- package-pure Hyde lowerers own the emitted Hyde strings for table-scoped Hyde
  commands only

No free table command helpers sit outside those IR objects.
...
The table window is created and updated by explicit Hyde kernel commands
generated through GUI-owned IR objects. The GUI owns transient
command-generation and saved-layout IR, while the kernel remains authoritative
for live scientific data.
...
All of these strings are generated by `TableIR` / `TableIRDiff`. The table does
not own the authoritative value being edited. The GUI may hold only transient
edit text and saved layout metadata long enough to generate the kernel command
or recreation source.
...
The save dialog receives the current `TableIR` directly when it opens. The
dialog stays thin, but it reads the existing table IR to derive the default
macro name and to emit recreation source without introducing a second save-state
mirror.
...
- track MDI window move/resize into `widget_ir.geometry`
- track per-data-column header resizes into `widget_ir.column_widths`
...
Layout state is intentionally represented in two persistence products:

- table recreation IR generated by `TableIR`
