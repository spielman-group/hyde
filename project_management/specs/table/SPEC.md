...
- `TableWidget` owns `TableState(HydeGuiState)` for table construction, recreation,
  layout capture, and background table requests
- `TableWidget` also owns `MutationState(HydeGuiState)` for live data mutations
- `TableCodec` owns all table-related Python generation
- `MutationCodec` owns generic mutation Python generation

No free table command helpers sit outside those state/codec pairs.

This two-state split is intentional. `TableState` remains table-specific, while
`MutationState` stays generic because Hyde expects other mutation-capable GUI
surfaces to reuse the same mutation path later.
...
- optional saved table layout through `geometry` and `column_widths`
...
The table window is created and updated by explicit Hyde kernel commands generated
through GUI-owned Hyde state objects. The GUI owns transient command-generation state
and saved layout state, while the kernel remains authoritative for live scientific
data.
...
being edited. The GUI may hold only transient edit text and saved layout metadata long
enough to generate the kernel command or recreation source.
...
This is intentional even though `session.toml` also stores table layout: macros must
be able to fully restore GUI layout when the user explicitly reopens a saved table.
...
to emit recreation source without introducing a second save-state mirror.
...
- track MDI window move/resize into `TableState.settings.geometry`
- track per-data-column header resizes into `TableState.settings.column_widths`
- skip first-fit layout sizing when recreated state already provides geometry or column widths
...
Layout state is intentionally duplicated in two places in the current design:

- table recreation state generated by `TableState`
- `session.toml` GUI session persistence

The recreation path and the session-restore path are not unified in this pass.
