...
- **Strict Figure-Window Boundary**: Only first-class `@hyde.figure` figures participate in Hyde's native MDI figure-window system in this deployment. Non-decorated figures remain ordinary kernel-side matplotlib figures and do not open Hyde figure windows.
...
- `project_management/ARCHITECTURE.md`, `project_management/specs/figure_window/SPEC.md`, `project_management/specs/IPC_PROTOCOL.md`, `issues/FigureWindow_prd.md`, and `project_management/specs/save_graphics_dialog/SPEC.md`: Tightened the figure-window contract so only first-class `@hyde.figure` figures enter Hyde's MDI window system.
...
- `project_management/specs/IPC_PROTOCOL.md`: Updated the high-level protocol contract so figure metadata and semantic figure edits travel over Jupyter `comm`, with only first-class `@hyde.figure` figures participating in the figure-window protocol.
